...
def cast_count_all(col):
    ...
    :param col: the collection you want to evaluate
    :type col: pymongo.collection.Collection
    :return collection_cast_counts: the tally of the forecast array lengths on the documnts in the collection
    :type: dict
    '''

    n = 0
    collection_cast_counts = {}
    # Go through each doc in the collection and count the number of items in the forecasts array, then add to
    # the tally for that count.
    for doc in col.find({}):
        n = count(doc)
        if n in collection_cast_counts:
            collection_cast_counts[n] += 1
...
def itslegit(instant):
    ...
    :param instant: the instant docuemnt to be legitimized
    :type instant: dictionary
    :return boolian: True if the document has 40 forecasts, otherwise False
    '''
